    ...
    <header className="bg-primary text-white shadow-lg sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <div className="flex items-center space-x-2 cursor-pointer" onClick={goHome}>
          ...
          <span className="font-bold text-xl tracking-tight hidden md:inline text-red-500">CultureAdvisory.org</span>
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 relative">
        ...
        <div className="max-w-xl w-full bg-white rounded-2xl shadow-xl overflow-hidden">
          <div className="bg-primary p-8 text-center">
            ...
            <p className="text-red-500 font-semibold">Organizational Alignment Intelligence</p>
          </div>
          <div className="p-8">
     ...
     return (
      <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <main className="flex-1 max-w-5xl mx-auto w-full px-4 py-8">
          ...
          {/* Co-Located Device Pass-Around Guide & Employee Privacy Info */}
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden mb-12">
            <div className="bg-primary p-6 text-white">
              <h2 className="text-xl font-bold flex items-center gap-2">
                ...
                Co-Located Device Pass-Around Flow Instructions
              </h2>
              <p className="text-sm text-slate-300 mt-1">
                Optimized for a fast, single-device, secure local collection. Perfect for managers and teams in the same office.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-100">
              ...
              <div className="p-6 md:p-8">
                <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
                  <span className="w-6 h-6 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center font-bold text-xs">M</span>
                  For managers / leaders
                </h3>
                <ul className="space-y-4 text-sm text-slate-600">
                  <li className="flex items-start gap-2.5">
                    <span className="font-semibold text-primary mt-0.5">Step 1:</span>
                    <span>Click <strong>Start Self Assessment</strong> below, choose <strong>"I am a Leader"</strong>, and complete your 15-question strategic assessment.</span>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <span className="font-semibold text-primary mt-0.5">Step 2:</span>
                    <span>Go to your dashboard, hand this notebook, tablet, or device to a staff member and ask them to complete their feedback.</span>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <span className="font-semibold text-primary mt-0.5">Step 3:</span>
                    <span>Staff can click <strong>"Pass Device to Next Staff"</strong> until at least 2 or 3 inputs are collected, then return the device to you to analyze the gap scores.</span>
                  </li>
                </ul>
              </div>
              ...
              <div className="p-6 md:p-8 bg-slate-50/50">
                <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
                  <span className="w-6 h-6 bg-green-100 text-green-600 rounded-full flex items-center justify-center font-bold text-xs">E</span>
                  For employees / staff
                </h3>
                <div className="bg-sky-50 border border-sky-100 text-sky-900 p-4 rounded-xl mb-4 text-xs font-semibold flex items-start gap-2.5">
                  ...
                  <div>
                    <h4 className="font-bold text-sky-900 mb-0.5">100% Secure & Anonymous Pledge</h4>
                    Employees do not have to worry. No email, name, registration, or tracking is logged. Your manager only gets aggregate cultural gap analysis summaries.
                  </div>
                </div>
                <p className="text-xs text-slate-500 leading-relaxed">
                  Your feedback is vital to bridge strategic alignment gaps in the organisation. When you complete compiling feedback on this device, your answers are safely added to the local compilation. No personal details leave this screen.
                </p>
              </div>
            </div>
          </div>
     ...
     return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center">
          ...
          <p className="text-slate-600 mb-6">Your anonymous feedback has been securely recorded. It will be aggregated with other staff responses to help leadership improve the culture.</p>
          
          {/* Notebook Pass-Around / Company size info notice */}
          {staffSubmissions.length === 2 ? (
            <div className="bg-blue-50 border border-blue-200 text-slate-700 rounded-xl p-4 mb-6 text-left text-xs leading-relaxed">
              <p className="font-bold text-blue-900 mb-1 flex items-center gap-1">
                <svg className="w-4 h-4 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                Company Participation Advisory:
              </p>
              Your company has now collected <strong>2 staff submissions</strong>. If this satisfies your company's list of participants, the manager may now load the dashboard to analyze results.
            </div>
          ) : (
            <div className="bg-slate-50 border border-slate-200 text-slate-600 rounded-xl p-4 mb-6 text-left text-xs leading-relaxed">
              <span className="font-medium text-slate-700">Device Pass-Around Tip:</span> If other colleagues still need to add their anonymous inputs to this device, hand it over and click "Pass Device to Next Staff".
            </div>
          )}

          <div className="space-y-3">
             <button onClick={() => setView('ROLE_SELECTION')} className="w-full py-3 bg-accent text-white rounded-xl hover:bg-sky-600 font-semibold transition-colors">
               Pass Device to Next Staff
             </button>
             <button onClick={() => setView('LEADER_DASHBOARD')} className="w-full py-3 bg-primary text-white rounded-xl hover:bg-slate-800 font-semibold transition-colors">
               Return to Leader Dashboard (Manager Only)
             </button>
             <button onClick={() => startAssessment(UserRole.STAFF)} className="w-full py-3 text-xs text-slate-400 hover:text-slate-600 transition-colors">
               Submit another staff response (for testing)
             </button>
          </div>
        </div>
        <Footer />
          ...
          {staffSubmissions.length > 0 && staffSubmissions.length < 3 && (
             <div className="bg-amber-50 border-l-4 border-amber-500 p-4 mb-8 rounded-r-lg shadow-sm">
               <div className="flex">
                 ...
                 <div className="ml-3">
                   <p className="text-sm text-amber-800 font-bold">Staff Participation Status & Analysis Advisory</p>
                   {staffSubmissions.length === 2 ? (
                     <p className="text-sm text-amber-700 mt-1">
                       You have collected exactly <strong>2 staff inputs</strong>. If your company size or evaluation cohort is small, there is no need to wait for further submissions. Please load the dashboard now and click <strong>"Generate Final Report"</strong> below to evaluate and analyze the cultural alignment gap with these current submissions.
                     </p>
                   ) : (
                     <p className="text-sm text-amber-700 mt-1">
                       Only <strong>{staffSubmissions.length} staff</strong> submission has been collected. While 3 submissions is ideal for statistical aggregation, you can still load the leader dashboard to perform an active evaluation on this current data if desired.
                     </p>
                   )}
                 </div>
               </div>
